...
void SerialCommand::readSerial() 
{
	...
	#endif
	{
		...
		#if defined( SERIALCOMMANDDEBUG ) || defined( SERIALECHO )
		...
		if (isprint(inChar))   // Only printable characters into the buffer
		{
			...
			#else
			buffer[bufPos++]=tolower(inChar);   // Put character into buffer
			#endif
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		...
		#else
		strncpy(CommandList[numCommand].command,tolower(command),SERIALCOMMANDBUFFER);
		#endif
